...
public class InfluenceConnection : MonoBehaviour
{
	public Transform bottomLeft, upperRight;
	public DisplayMap display;
	...
	List<List<Influencer>> _lowLevelIMs;
	...
	void Awake()
	{
		_lowLevelIMs = new List<List<Influencer>>();
		...
		for (int s = 0; s < squadNumbers; ++s)
		{
			...
			_lowLevelIMs.Add(new List<Influencer>());
			for (int i = 0; i < lowLevelMaps.Length; ++i)
			{
				...
				_lowLevelIMs[s].Add(im_obj.AddComponent<Influencer>());
	...
	void InstallInfluenceMap(Influencer im, int idx)
	{
	...
	public void RegisterPropagator(Propogation p, string type, int squad_no)
	{
		bool found = false;
		for (int i = 0; i < lowLevelMaps.Length; ++i)
		{
	...
	public void DeadPropagator(Propogation p, string type, int squad_no)
	{
		bool found = false;
		for (int i = 0; i < lowLevelMaps.Length; ++i)
		{
